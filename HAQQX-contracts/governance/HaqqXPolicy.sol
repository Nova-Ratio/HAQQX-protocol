...
/* @title HaqqXPolicy
 * @notice Intermediates between the dex mechanism inside HaqqXSwapDex and the top-level
 ...
 *         method in HaqqXSwapDex. Policy can be either through governance resolutions, 
...
contract HaqqXPolicy is IHaqqXMaster {
    ...
    event HaqqXGovernAuthority (address ops, address treasury, address emergency);
     ...
     *               HaqqXSwapDex contract).
     * @param cmd The command being called on the minion's protocolCmd() function. */
    event HaqqXResolutionOps (address minion, bytes cmd);

    /* @notice Emitted whenever treasury authority runs a protocol command. 
     * @param minion The underlying receiver of the protocol command (i.e. the 
     *               HaqqXSwapDex contract).
     * @param sudo If true, calls the command on HaqqXSwapDex with elevated privilege
     * @param cmd The command being called on the minion's protocolCmd() function. */
    event HaqqXResolutionTreasury (address minion, bool sudo, bytes cmd);
    ...
    event HaqqXEmergencyHalt (address minion, string reason);
     ...
     *                   below). */
    event HaqqXPolicySet (address conduit, uint16 proxyPath, PolicyRule);
     ...
     * @param PolicyRule The policy rules set for this conduit (see PolicyRule comments
     *                   below). */
    event HaqqXPolicyForce (address conduit, uint16 proxyPath, PolicyRule);
     ...
     *               details. */
    event HaqqXPolicyEmergency (address conduit, string reason);
     ...
     *            1) Invoke partial halt on the underlying HaqqXSwapDex
    ...
    /* @param dex Underlying HaqqXSwapDex contract */
    constructor (address dex) {
        require(dex != address(0) && HaqqXSwapDex(dex).acceptHaqqXDex(), "Invalid HaqqXSwapDex");
        dex_ = dex;
     ...
     *         underlying HaqqXSwapDex contract. 
     *
     * @param minion The address of the underlying HaqqXSwapDex contract the command is
     *               called on.
    ...
    function opsResolution (address minion, uint16 proxyPath,
        ...
        emit HaqqXResolutionOps(minion, cmd);
        IHaqqXMinion(minion).protocolCmd(proxyPath, cmd, false);
    }
     ...
     *               called on.
     * @param proxyPath The proxy sidecar index the policy calls
     * @param sudo   If true, runs the call on HaqqXSwapDex with elevated privilege
     * @param cmd    The content of the command passed to the protocolCmd() method. */
    function treasuryResolution (address minion, uint16 proxyPath,
                                 bytes calldata cmd, bool sudo)
        treasuryAuth public {
        emit HaqqXResolutionTreasury(minion, sudo, cmd);
        IHaqqXMinion(minion).protocolCmd(proxyPath, cmd, sudo);
    }

    /* @notice An out-of-band emergency measure to protect funds in the HaqqXSwapDex 
     *         contract in case of a security issue. It works by disabling all the proxy
     *         contracts in HaqqXSwapDex (and disabling swap()'s in the hotpath), besides
     ...
     *
     * @param minion The address of the underlying HaqqXSwapDex contract.
     * @param reason The stated reason for invoking the emergency policy update with 
     *               details. */
    function emergencyHalt (address minion, string calldata reason)
        emergencyAuth public {
        emit HaqqXEmergencyHalt(minion, reason);

        bytes memory cmd = ProtocolCmd.encodeHotPath(false);
        IHaqqXMinion(minion).protocolCmd(HaqqXSlots.COLD_PROXY_IDX, cmd, true);
        
        cmd = ProtocolCmd.encodeSafeMode(true);
        IHaqqXMinion(minion).protocolCmd(HaqqXSlots.COLD_PROXY_IDX, cmd, true);
    }

    /* @notice HaqqX policy rules are set on a per address basis. Each address 
     *         corresponds to a smart contract, which is authorized to invoke one or 
     *         more protocol commands on the underlying HaqqXSwapDex contract. 
     * 
     ...
     *         HaqqXSwapDex. Authority for the specific protocol command is checked 
    ...
    function setPolicy (address conduit, uint16 proxyPath,
        ...
        rules_[key] = policy;
        emit HaqqXPolicySet(conduit, proxyPath, policy);
    }
    ...
    function forcePolicy (address conduit, uint16 proxyPath, PolicyRule calldata policy)
        ...
        bytes32 key = rulesKey(conduit, proxyPath);
        rules_[key] = policy;
        emit HaqqXPolicyForce(conduit, proxyPath, policy);
    }
    ...
    function emergencyReset (address conduit, uint16 proxyPath,
        ...
        emit HaqqXPolicyEmergency(conduit, reason);
    }
    ...
    function acceptsHaqqXAuthority() public override pure returns (bool) { return true; }
