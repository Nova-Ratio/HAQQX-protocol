 ...
 *         parsing the internal state of a HaqqXSwapDex contract. 
 *
 * @dev Nothing in this contract can't be done by directly accessing readSlot() on the 
 *      HaqqXSwapDex contrct. However this provides a more convienent interface with ergonomic
...
contract HaqqXQuery {
    ...
    /* @param dex The address of the HaqqXSwapDex contract. */    
    ...
    function queryPoolParams (address base, address quote, uint256 poolIdx)
        ...
        bytes32 key = PoolSpecs.encodeKey(base, quote, poolIdx);
        bytes32 slot = keccak256(abi.encode(key, HaqqXSlots.POOL_PARAM_SLOT));
        uint256 valOne = HaqqXSwapDex(dex_).readSlot(uint256(slot));
    ...
    function queryPoolTemplate (uint256 poolIdx)
        public view returns (PoolSpecs.Pool memory pool) {
        bytes32 slot = keccak256(abi.encode(poolIdx, HaqqXSlots.POOL_TEMPL_SLOT));
        uint256 valOne = HaqqXSwapDex(dex_).readSlot(uint256(slot));
    ...
    function queryKnockoutPivot (address base, address quote, uint256 poolIdx,
        ...
        bytes32 slot = keccak256(abi.encodePacked(key, HaqqXSlots.KO_PIVOT_SLOT));
        uint256 val = HaqqXSwapDex(dex_).readSlot(uint256(slot));
     ...
     * @return fee The accumulated range order fee at the knockout time (in units of haqq 
    ...
    function queryKnockoutMerkle (address base, address quote, uint256 poolIdx,
        ...
        bytes32 poolHash = PoolSpecs.encodeKey(base, quote, poolIdx);
        bytes32 key = KnockoutLiq.encodePivotKey(poolHash, isBid, tick);
        bytes32 slot = keccak256(abi.encodePacked(key, HaqqXSlots.KO_MERKLE_SLOT));
        uint256 val = HaqqXSwapDex(dex_).readSlot(uint256(slot));
    ...
    function queryKnockoutPos (KnockoutLiq.KnockoutPosLoc memory loc,
        ...
        bytes32 slot = keccak256(abi.encodePacked(key, HaqqXSlots.KO_POS_SLOT));
        uint256 val = HaqqXSwapDex(dex_).readSlot(uint256(slot));
    ...
    function queryRangePosition (address owner, address base, address quote,
        ...
        bytes32 slot = keccak256(abi.encodePacked(posKey, HaqqXSlots.POS_MAP_SLOT));
        uint256 val = HaqqXSwapDex(dex_).readSlot(uint256(slot));
    ...
    /* @notice Queries and returns the state of a single haqq order liquidity position.
     *
     * @param owner The address that owns the liquidity position
     * @param base The base token address of the pair
     * @param quote The quote token address of the pair
     * @param poolIdx The index of the pool type
     *
     * @return seeds The total amount of haqq liquidity seeds in the position in units
     *               of rewards deflated sqrt(X*Y) liquidity
     * @return timestamp The block time that the liquidity is stamped with from latest mint */
    function queryHaqqPosition (address owner, address base, address quote,
                                   uint256 poolIdx)
        public view returns (uint128 seeds, uint32 timestamp) {
        bytes32 poolHash = PoolSpecs.encodeKey(base, quote, poolIdx);
        bytes32 posKey = keccak256(abi.encodePacked(owner, poolHash));
        bytes32 slot = keccak256(abi.encodePacked(posKey, HaqqXSlots.AMB_MAP_SLOT));
        uint256 val = HaqqXSwapDex(dex_).readSlot(uint256(slot));
    ...
    /* @notice Queries and returns the total haqq liquidity rewards accumulated by a
     ...
     * @param upperTick The 24-bit price tick the lower end of the liquidity range
     *
     * @return The total accumulated rewards in the form of haqq sqrt(X*Y) liquidity */
    ...
    /* @notice Queries and returns the liquidity and tokens held by a single haqq
     *         liquidity position
     *
     * @param owner The address that owns the liquidity position
     * @param base The base token address of the pair
     ...
     * @param poolIdx The index of the pool type
     *
     * @return liq The total amount of haqq sqrt(X*Y) liquidity 
     ...
     *                  curve price. */
    function queryHaqqTokens (address owner, address base, address quote,
                                 uint256 poolIdx)
        public view returns (uint128 liq, uint128 baseQty, uint128 quoteQty) {
        (uint128 seeds, ) = queryHaqqPosition(owner, base, quote, poolIdx);
        CurveMath.CurveState memory curve = queryCurve(base, quote, poolIdx);
        return convertSeedsToLiq(curve, seeds);
    }
