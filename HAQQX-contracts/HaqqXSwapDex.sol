...
/* @title HaqqXSwap exchange contract
 * @notice Top-level HaqqXSwap contract. Contains all public facing methods and state
 ...
 *         with. */
contract HaqqXSwapDex is HotPath, IHaqqXMinion {
    ...
    constructor() {
        ...
        hotPathOpen_ = true;
        proxyPaths_[HaqqXSlots.BOOT_PROXY_IDX] = address(new BootPath());
    }
    ...
    function userCmdRelayer (uint16 callpath, bytes calldata cmd,
        ...
        reEntrantAgent(HaqqXRelayerCall(callpath, cmd, conds, relayerTip), signature)
     ...
     *         footprint of HaqqXSwapDex down. See SlotLocations.sol and QueryHelper.sol 
     ...
     *         a valid HaqqXSwapDex contract. */
    function acceptHaqqXDex() pure public returns (bool) { return true; }
}


/* @notice Alternative constructor to HaqqXSwapDex that's more convenient. However
 *     the deploy transaction is several hundred kilobytes and will get droppped by 
 *     geth. Useful for testing environments though. */
contract HaqqXSwapDexSeed  is HaqqXSwapDex {
    
    constructor() {
        proxyPaths_[HaqqXSlots.LP_PROXY_IDX] = address(new WarmPath());
        proxyPaths_[HaqqXSlots.COLD_PROXY_IDX] = address(new ColdPath());
        proxyPaths_[HaqqXSlots.LONG_PROXY_IDX] = address(new LongPath());
        proxyPaths_[HaqqXSlots.MICRO_PROXY_IDX] = address(new MicroPaths());
        proxyPaths_[HaqqXSlots.FLAG_CROSS_PROXY_IDX] = address(new KnockoutFlagPath());
        proxyPaths_[HaqqXSlots.KNOCKOUT_LP_PROXY_IDX] = address(new KnockoutLiqPath());
        proxyPaths_[HaqqXSlots.SAFE_MODE_PROXY_PATH] = address(new SafeModePath());
    }
}
