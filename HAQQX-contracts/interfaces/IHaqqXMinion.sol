...
/* @notice Simple interface that defines the surface between the HaqqXSwapDex
 *         itself and protocol governance and policy. All governance actions are
 *         are executed through the single protocolCmd() method. */
interface IHaqqXMinion {

    /* @notice Calls a general governance authorized command on the HaqqXSwapDex contract.
     *
...
/* @notice Interface for a contract that directly governs a HaqqXSwap dex contract. */
interface IHaqqXMaster {
    /* @notice Used to validate governance contract to prevent authority transfer to an
     *         an invalid address or contract. */
    function acceptsHaqqXAuthority() external returns (bool);
}