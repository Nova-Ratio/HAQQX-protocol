...
contract MicroPaths is MarketSequencer {
     ...
     * @param seed The haqq liquidity seeds in the current curve.
     * @param conc The active in-range concentrated liquidity in the current curve.
     * @param seedGrowth The cumulative haqq seed deflator in the current curve.
     ...
     *                   action. 
     * @return seedOut The updated haqq seed liquidity on the curve.
    ...
    /* @notice Burns liquidity from an haqq liquidity position on a single curve.
     *
     * @param price The price of the curve. Represented as the square root of the exchange
     *              rate in Q64.64 fixed point
     * @param seed The haqq liquidity seeds in the current curve.
     ...
     *                   action. 
     * @return seedOut The updated haqq seed liquidity on the curve. */     
    function burnHaqq (uint128 price, uint128 seed, uint128 conc,
                          uint64 seedGrowth, uint64 concGrowth,
        ...
        (baseFlow, quoteFlow) = burnHaqq(curve, liq, poolHash, lockHolder_);
        
        seedOut = curve.haqqSeeds_;
    }

    /* @notice Mints liquidity from an haqq liquidity position on a single curve.
     *
     * @param price The price of the curve. Represented as the square root of the exchange
     *              rate in Q64.64 fixed point
     * @param seed The haqq liquidity seeds in the current curve.
     ...
     *                   action. 
     * @return seedOut The updated haqq seed liquidity on the curve. */         
    function mintHaqq (uint128 price, uint128 seed, uint128 conc,
                          uint64 seedGrowth, uint64 concGrowth,
                          uint128 liq, bytes32 poolHash)
        public payable returns (int128 baseFlow, int128 quoteFlow, uint128 seedOut) {
        CurveMath.CurveState memory curve;
        ...
        (baseFlow, quoteFlow) = mintHaqq(curve, liq, poolHash, lockHolder_);
     ...
     * @return seedOut The haqq liquidity seeds in the curve after the swap completes
     * @return concOut The active in-range concentrated liquidity in the curve post-swap
     * @return haqqOut The cumulative haqq seed deflator on the curve post-swap.
    ...
    function acceptHaqqXProxyRole (address, uint16 slot) public pure returns (bool) {
        return slot == HaqqXSlots.MICRO_PROXY_IDX;
    }
}
