...
contract BootPath is StorageLayout {
    ...
    function upgradeProxy (bytes calldata cmd) private {
        ...
        require(proxyIdx != HaqqXSlots.BOOT_PROXY_IDX, "Cannot overwrite boot path");
        require(proxy == address(0) || proxy.code.length > 0, "Proxy address is not a contract");

        emit HaqqXEvents.UpgradeProxy(proxy, proxyIdx);
        proxyPaths_[proxyIdx] = proxy;        

        if (proxy != address(0)) {
            bool doesAccept = BootPath(proxy).acceptHaqqXProxyRole(address(this), proxyIdx);
    ...
    function acceptHaqqXProxyRole (address, uint16) public pure virtual returns (bool) {
        return false;
    }
}
