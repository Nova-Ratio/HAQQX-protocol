...
/* @title HaqqSwap exchange contract
 * This is the only contract users needs to directly interact.*/
contract HaqqSwapDex is HotPath, IHaqqMinion {
    ...
    constructor() {
        ...
        hotPathOpen_ = true;
        proxyPaths_[HaqqSlots.BOOT_PROXY_IDX] = address(new BootPath());
    }
     ...
     * 
     * @param callpath The proxy sidecar callpath called into.
     * @param cmd      The arbitrary byte calldata corresponding to the command.
    ...
    /* @notice Calls userCmd for external scenarios with additional parameters
     *
     * @param callpath The index of the proxy sidecar the command is being called on.
     * @param cmd The arbitrary call data the client is calling the proxy sidecar.
    ...
    function userCmdRelayer (uint16 callpath, bytes calldata cmd,
        ...
        reEntrantAgent(HaqqRelayerCall(callpath, cmd, conds, relayerTip), signature)
     ...
     *         footprint of HaqqSwapDex down. See SlotLocations.sol and QueryHelper.sol 
     ...
     *         a valid HaqqSwapDex contract. */
    function acceptHaqqDex() pure public returns (bool) { return true; }
}

/* @notice Alternative constructor to HaqqSwapDex that's more convenient. However
 *     the deploy transaction is several hundred kilobytes and will get droppped by 
 *     geth. Useful for testing environments though. */
contract HaqqSwapDexSeed  is HaqqSwapDex {
    
    constructor() {
        proxyPaths_[HaqqSlots.COLD_PROXY_IDX] = address(new ColdPath());
        proxyPaths_[HaqqSlots.LP_PROXY_IDX] = address(new WarmPath());        
        proxyPaths_[HaqqSlots.LONG_PROXY_IDX] = address(new LongPath());
        proxyPaths_[HaqqSlots.MICRO_PROXY_IDX] = address(new MicroPaths());
        proxyPaths_[HaqqSlots.FLAG_CROSS_PROXY_IDX] = address(new KnockoutFlagPath());
        proxyPaths_[HaqqSlots.KNOCKOUT_LP_PROXY_IDX] = address(new KnockoutLiqPath());
        proxyPaths_[HaqqSlots.SAFE_MODE_PROXY_PATH] = address(new SafeModePath());
    }
}
